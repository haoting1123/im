	...
	<view class="page">
		...
		<view class="uni-list">
			<block v-for="(message, index) in historyMessageList" :key="index">
				<view class="uni-list-cell" hover-class="uni-list-cell-hover" @tap="goDetail(message)">
					<view class="uni-triplex-row">
						<view class="uni-triplex-left">
							...
							<text class="uni-h4" style="font-size: 28upx; color:#333333;" v-else>{{ message.senderId === userInfo.jid ? '我' : channelName }}</text>
						...
						<view class="uni-triplex-right message-create-at">
							<text class="uni-h5">{{ message.createTimeStr }}</text>
						</view>
					</view>
				</view>
			</block>
...
export default {
	...
	onLoad(option) {
		...
		this.searchKeyword = option.searchKeyword;
	...
	computed: {
		...mapState({
			...
			searchCurrentChannel: state => state.message.searchCurrentChannel,
		})
	},
	watch: {
	...
	onUnload() {
		this.SET_SEARCH_CURRENT_CHANNEL(null)
	},
	methods: {
		...mapMutations(['CLEAR_HISTORY_MESSAGE_LIST','SET_SEARCH_CURRENT_CHANNEL']),
		...mapActions(['getAllHistoryMessage','getAllHistoryMessageBySearchKey']),
		initFirstMessage(isLoadMore, limit) {
			if(!limit){
				limit = 20;
			}
			let data = {
			...
			this.getAllHistoryMessage(data).then(resp => {
				...
				if (resp && resp.length > 0) {
					...
					resultData.forEach(item => {
						...
						item.createTimeStr = this.getFriendlyTime(item.createTime);
						if (item.fileSize > 0) {
							item.filePath =
								this.hostUrl +
		...
		searchKeyClickMessage(maxCreateAt) {
			let data = {
				channelId: this.channelId,
				maxCreateAt: maxCreateAt
			};
			this.getAllHistoryMessageBySearchKey(data).then(resp => {
				// console.log("请求成功")
				// console.log(JSON.stringify(resp));
				if (resp && resp.length > 0) {
					this.hasMore = true;
		...
		toScrollBottom(){
			...
			query.selectAll('.uni-list-cell').boundingClientRect();
			query.exec(function(res) {
			    ...
			    if (that.style.mitemHeight > that.style.contentViewHeight) {
					uni.pageScrollTo({
						scrollTop: (that.style.mitemHeight - that.style.contentViewHeight + 460),
						duration: 300
					});
			    }
			});
		...
		goDetail(e) {
			if(this.searchKeyword){
				...
				this.searchKeyClickMessage(e.createTime)
			}
		},
	}
};
