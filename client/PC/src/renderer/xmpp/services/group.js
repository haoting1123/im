...
export function enterGroupXMPP (data) {
  xmppClient.send(
    xml('presence', { from: data.userJid, to: `${data.groupJid}/${data.userJid.split('@')[0]}` })
  )
}
...
export function groupInviteListenerXMPP (callback) {
  // globalStatus['message::stanza-id'] = callback
...
export function filterGroupMemberXMPP (stanza) {
  let x2js = new X2js()
  let group = x2js.xml2js(stanza.toString()).iq
  console.log(x2js.xml2js(stanza.toString()))
  let result = {
    jid: group._from,
    members: []
  }
  if (Object.prototype.toString.call(group.query.item) === '[object Array]') {
    group.query.item.forEach(item => {
      let obj = {
        jid: item._jid,
        affiliation: item._affiliation,
        name: item.vCard.name,
        photo: item.vCard.photo,
        sex: item.vCard.sex
      }
...
export function createGroupXMPP (data, callback) {
  const id = Math.random().toString(36).substr(2, 10)
  const date = new Date()
  const groupName = `${date.getFullYear()}${(date.getMonth() + 1).toString().padStart(2, '0')}${date.getDate().toString().padStart(2, '0')}${date.getHours().toString().padStart(2, '0')}${date.getMinutes().toString().padStart(2, '0')}${date.getSeconds().toString().padStart(2, '0')}${Math.floor(Math.random() * 10000 + 1)}`
  xmppClient.send(
