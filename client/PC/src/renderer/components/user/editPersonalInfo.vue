<template>
    <el-form :model="myInfoModel" :rules="formRules" class="el-dialog-form"
      ref="userForm" label-width="90px" label-position="right" size="small"  @submit.native.prevent>
...
import { myBASE_API } from '@/utils/url'
...
export default {
  ...
  data() {
    return {
      ...
      newAvatarUrl: '',
      oldNickname: '',
      uploadAvatarUrl: myBASE_API() + '/users/me/avatar',
        ...
        .then(response => {
          ...
          if(myAvatarUrl != null && myAvatarUrl.trim() !== '') {
            ...
            if(!myAvatarUrl.startsWith('http:') && !myAvatarUrl.startsWith('https:')) {
              const userId = JSON.parse(sessionStorage.getItem('currentUser')).id
              this.myInfoModel.avatarUrl = myBASE_API() + '/users/' + userId + '/avatar?width=80&height=80&rdm=' + Math.random()
            }
          }
